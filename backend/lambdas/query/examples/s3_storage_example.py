...
Member D's delete endpoint calls `StorageClient.delete(keys)` to remove objects
from storage after the DB records are gone. The keys are already S3 object keys
(`originals/...`, `thumbnails/...`), so no URL parsing is needed.
...
class S3StorageClient(StorageClient):
    ...
    def delete(self, keys: list[str]) -> None:
        ...
        if not objects:
            return
        # S3 supports deleting up to 1000 keys per call.
