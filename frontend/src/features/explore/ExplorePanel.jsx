...
export default function ExplorePanel({ items, queryState, selectedFileIds, onQueryStart, onQueryResult, onToggle, onStatus, sessionKey }) {
  ...
  return <section className="explore-layout" aria-busy={queryState === "loading"}>
    <aside className="explore-controls" aria-label="Explore search controls">
      ...
      <details className="advanced-lookup"><summary>Advanced lookup</summary><form className="stack" onSubmit={(event) => { event.preventDefault(); run(async () => { const data = await queryByThumbnail(thumbnailKey); return { results: data.original_key ? [data.original_key] : [], items: data.item ? [data.item] : [] }; }); }}><Field label="Thumbnail key or URL"><input value={thumbnailKey} onChange={(event) => setThumbnailKey(event.target.value)} /></Field><button type="submit" className="secondary-button" disabled={!thumbnailKey.trim() || queryState === "loading"}>Find original</button></form></details>
    </aside>
